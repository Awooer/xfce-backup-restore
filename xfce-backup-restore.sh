...
# for backup use "sh xfce-backup-restore.sh backup"
# for restore from backup use "sh xfce-backup-restore.sh restore"
# while using restore, xfce4-backup.tar.gz have to be in the same directory with script
# DONT RUN THIS SCRIPT AS ROOT
# Author's github https://github.com/Awooer
MODE=$1 # mode
...
if [ "$MODE" = backup ]; then
    ...
    echo "error '$MODE' is not a argument use 'backup' or 'restore'"
fi
